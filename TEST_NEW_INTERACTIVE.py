...
def upload_to_supabase(image_bytes, file_name):
    try:
        supabase.storage.from_("employee-photos").upload(path=file_name, file=image_bytes, file_options={"content-type": "image/jpeg"})
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS employees (id SERIAL PRIMARY KEY, first_name TEXT, last_name TEXT, dept_name TEXT, is_active INTEGER DEFAULT 1)''')
    ...
    if menu == "📺 LIVE VISION":
        ...
        with st.form("enroll_form"):
            ...
            if st.form_submit_button("✨ COMMIT TO DATABASE"):
                ...
                conn.commit(); cur.close(); conn.close(); st.success("✅ Employee enrolled successfully!"); st.balloons()
        ...
        if 'res' in st.session_state and st.session_state['res']:
            with st.form("mod_form"):
                ...
                if st.form_submit_button("💾 OVERWRITE"):
                    ...
                    conn.commit(); cur.close(); conn.close(); st.success("✅ Profile updated!"); st.rerun()
        ...
        if st.button("🚨 DELETE ALL DATA"):
            conn = get_db_connection(); cur = conn.cursor()
            cur.execute("DELETE FROM employees"); cur.execute("DELETE FROM attendance"); conn.commit(); conn.close(); st.error("Data Wiped")
